...
job_index = 0
job_settings = {}
for trial in range(10):  # number of random trials
    for i in range(20):  # number of outer folds
        for l1 in lmbdas:  # L1
            job_settings[job_index] = (trial, i, l1)
            job_index += 1
trial, outer, lmbda = job_settings[int(k)]

# import train and test data
train_df = pd.read_csv(f"folds_CDanc/train_{outer}_{trial}.csv")
test_df = pd.read_csv(f"folds_CDanc/test_{outer}_{trial}.csv")
...
# save dataframe
pred_df.to_csv(f"CDanc_pred_{outer}_{lmbda}_{trial}.csv", index=False)
